...
<%
	...
	count = dao.getCount2(); // 데이터베이스에 저장된 총 갯수
	...
	if (count > 0) {
		...
		list = dao.selectReviewPs4(startRow, endRow, user_id);
	}
%>
<html>
<head>
		...
		<div>
			...
			<tr>
				...
				<td>
									...
									<input type="button" value="삭제" onclick="location.href='pagemove?command=deleteform&rv_no=<%=board.getRv_no()%>&rv_platform=<%=board.getRv_platform()%>'">
									</td>
			</tr>
			<%
					}
			...
			<tr>
				<td colspan="6" align="center" style="text-align: center">
					...
					%>
								<a href="pagemove?command=user_dashboard_pop_ps4&pageNum=<%=startPage - 10%>">[이전]</a>	
					<%			
							}
					...
					%>
									<a href="pagemove?command=user_dashboard_pop_ps4&pageNum=<%=i%>">[<%=i %>]</a>
					<%	
								}
							} // for end
					...
					%>
								<a href="pagemove?command=user_dashboard_pop_ps4&pageNum=<%=startPage + 10 %>">[다음]</a>
					<%			
							}
						}
					%>
