...
// 해당 페이지에서 시작할 레코드 / 마지막 레코드
	...
	count = dao.getCount1(); // 데이터베이스에 저장된 총 갯수
	...
	list = dao.selectReviewSwitch(startRow, endRow, user_id);
		}
	%>
<html>
<head>
...
<body>
				...
				<tbody>
					...
					<tr>
						...
						<input
							...
							onclick="location.href='pagemove?command=deleteform&rv_no=<%=board.getRv_no()%>&rv_platform=<%=board.getRv_platform()%>&user_id=<%=user_id %>&pageNum=<%=pageNum%>'">
						</td>
					</tr>
					<%
						}
					...
					<tr>
						<td colspan="6" align="center" style="text-align: center">
							...
							%> <a
							href="pagemove?command=user_dashboard_pop_switch&pageNum=<%=startPage - 10%>">[이전]</a>
							<%
								}
							...
							%> <a href="pagemove?command=user_dashboard_pop_switch&pageNum=<%=i%>">[<%=i%>]
						</a> <%
 	}
 } // for end
 ...
 %> <a
							href="pagemove?command=user_dashboard_pop_switch&pageNum=<%=startPage + 10%>">[다음]</a>
							<%
								}
							}
							%>
